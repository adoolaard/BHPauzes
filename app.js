...
function createDefaultState() {
  ...
  return {
    ...
    ui: {
      ...
      activeBarId: event.bars[0].id,
      activeTab: 'dashboard',
      showArchived: false,
      drafts: {
        employee: { name: '', shiftId: event.bars[0].shifts[0]?.id || '' },
...
let state = loadState();
let showBarSelector = activeEvent().bars.length > 1;
...
function migrateV1(data) {
  ...
  const bar = createBar('Hoofdbar');
  bar.shifts = Array.isArray(data?.shifts) ? data.shifts : createDefaultShifts();
  bar.employees = Array.isArray(data?.employees) ? data.employees : [];
  bar.breaks = Array.isArray(data?.breaks) ? data.breaks : [];
  bar.maxConcurrentBreaks = Number(data?.event?.maxConcurrentBreaks || 3);
  const event = {
    id: uid('event'),
  ...
  return normalizeState({ ...fallback, events: [event], ui: { ...fallback.ui, activeEventId: event.id, activeBarId: bar.id }, settings: { ...fallback.settings, ...(data?.settings || {}) } });
}

function migrateBarFirstV2(data) {
  const fallback = createDefaultState();
  const eventMap = new Map();
  (Array.isArray(data?.bars) ? data.bars : []).forEach((oldBar) => {
    (Array.isArray(oldBar.events) ? oldBar.events : []).forEach((oldEvent) => {
      const key = `${oldEvent.name || 'Naamloos evenement'}|${oldEvent.date || formatDateInput()}|${Boolean(oldEvent.archived)}`;
      if (!eventMap.has(key)) {
        eventMap.set(key, {
          id: oldEvent.id || uid('event'),
          name: oldEvent.name || 'Naamloos evenement',
          date: oldEvent.date || formatDateInput(),
          archived: Boolean(oldEvent.archived),
          createdAt: oldEvent.createdAt || now().toISOString(),
          archivedAt: oldEvent.archivedAt || null,
          bars: [],
        });
      }
      const bar = createBar(oldBar.name || 'Hoofdbar');
      bar.createdAt = oldBar.createdAt || bar.createdAt;
      bar.maxConcurrentBreaks = Number(oldEvent.maxConcurrentBreaks || 3);
      bar.shifts = Array.isArray(oldEvent.shifts) && oldEvent.shifts.length ? oldEvent.shifts : createDefaultShifts();
      bar.employees = Array.isArray(oldEvent.employees) ? oldEvent.employees : [];
      bar.breaks = Array.isArray(oldEvent.breaks) ? oldEvent.breaks : [];
      eventMap.get(key).bars.push(bar);
    });
  });
  const events = Array.from(eventMap.values());
  const event = events.find((item) => !item.archived) || events[0] || fallback.events[0];
  const bar = event.bars[0];
  return normalizeState({ ...fallback, events: events.length ? events : fallback.events, ui: { ...fallback.ui, activeEventId: event.id, activeBarId: bar.id }, settings: { ...fallback.settings, ...(data?.settings || {}) } });
}

function normalizeState(data) {
  if (Array.isArray(data?.bars) && !Array.isArray(data?.events)) return migrateBarFirstV2(data);
  const fallback = createDefaultState();
  const normalized = {
    version: 2,
    events: Array.isArray(data?.events) && data.events.length ? data.events : fallback.events,
  ...
  normalized.events = normalized.events.map((event) => ({
    ...
    bars: (Array.isArray(event.bars) && event.bars.length ? event.bars : [createBar('Hoofdbar')]).map((bar) => ({
      id: bar.id || uid('bar'),
      name: bar.name || 'Naamloze bar',
      createdAt: bar.createdAt || now().toISOString(),
      maxConcurrentBreaks: Number(bar.maxConcurrentBreaks || event.maxConcurrentBreaks || 3),
      shifts: Array.isArray(bar.shifts) && bar.shifts.length ? bar.shifts : createDefaultShifts(),
      employees: Array.isArray(bar.employees) ? bar.employees : [],
      breaks: Array.isArray(bar.breaks) ? bar.breaks.map((entry) => ({ notificationsSent: [], alertsSent: [], ...entry })) : [],
    })),
  }));
  const activeEvents = normalized.events.filter((event) => !event.archived);
  if (!normalized.events.some((event) => event.id === normalized.ui.activeEventId) || normalized.events.find((event) => event.id === normalized.ui.activeEventId)?.archived) {
    normalized.ui.activeEventId = (activeEvents[0] || normalized.events[0]).id;
  }
  const event = normalized.events.find((item) => item.id === normalized.ui.activeEventId) || normalized.events[0];
  if (!event.bars.some((bar) => bar.id === normalized.ui.activeBarId)) normalized.ui.activeBarId = event.bars[0].id;
...
function activeEvent() {
  return state.events.find((event) => event.id === state.ui.activeEventId) || state.events.find((event) => !event.archived) || state.events[0];
}

function activeBar() {
  const event = activeEvent();
  return event.bars.find((bar) => bar.id === state.ui.activeBarId) || event.bars[0];
}

function activeEvents() {
  return state.events.filter((event) => !event.archived);
}

function getShift(employee) {
  return activeBar().shifts.find((shift) => shift.id === employee.shiftId);
}

function getEmployeeBreaks(employeeId) {
  return activeBar().breaks.filter((entry) => entry.employeeId === employeeId).sort((a, b) => new Date(a.startedAt) - new Date(b.startedAt));
}

function getActiveBreak(employeeId) {
  return activeBar().breaks.find((entry) => entry.employeeId === employeeId && !entry.endedAt);
}

function getActiveBreaks() {
  return activeBar().breaks.filter((entry) => !entry.endedAt).sort((a, b) => new Date(a.expectedReturnAt) - new Date(b.expectedReturnAt));
}
...
function progressStats() {
  const bar = activeBar();
  const totalRequired = bar.employees.reduce((sum, employee) => sum + employee.requiredBreakMinutes, 0);
  const totalCompleted = bar.employees.reduce((sum, employee) => sum + completedMinutes(employee.id), 0);
  const complete = bar.employees.filter((employee) => remainingMinutes(employee) === 0).length;
  const partial = bar.employees.filter((employee) => completedMinutes(employee.id) > 0 && remainingMinutes(employee) > 0).length;
  const none = bar.employees.filter((employee) => completedMinutes(employee.id) === 0 && !getActiveBreak(employee.id)).length;
...
function expectedProgressPercentage() {
  const bar = activeBar();
  if (!bar.employees.length) return 0;
  const current = now().getHours() * 60 + now().getMinutes();
  const values = bar.employees.map((employee) => {
...
function adviceList() {
  const bar = activeBar();
  const room = Math.max(0, bar.maxConcurrentBreaks - getActiveBreaks().length);
  if (!room) return [];
  const current = now();
  return bar.employees
...
function shouldShowBarSelection() {
  return activeEvent().bars.length > 1 && (showBarSelector || !state.ui.activeBarId);
}
...
function renderBarSelection() {
  ...
  document.querySelector('main').innerHTML = `<section id="barSelector" class="tab-panel active">
    <div class="card hero-card">
      ...
      <div class="grid">${activeEvent().bars.map((bar) => `<button type="button" class="selector-button ${bar.id === state.ui.activeBarId ? 'active' : ''}" data-select-bar="${bar.id}">${escapeHtml(bar.name)}<span>${activeEvent().name}</span></button>`).join('')}</div>
  ...
  $('#dashboard').innerHTML = `
    <div class="card">
      <div class="card-title">
        <div><h2>${escapeHtml(event.name)}</h2><p class="muted small">${escapeHtml(activeBar().name)} · ${event.date} · max. ${activeBar().maxConcurrentBreaks} tegelijk op pauze</p></div>
        <span class="status-pill ${statusClass}">${stats.status}</span>
      </div>
      ${renderWarningStrip()}
      <div class="grid two four">${stat('Medewerkers', activeBar().employees.length)}${stat('Volledig klaar', stats.complete)}${stat('Gedeeltelijk klaar', stats.partial)}${stat('Zonder pauze', stats.none)}</div>
    </div>
    <div class="desktop-columns">
      ...
      <div class="card"><div class="card-title"><h3>Slim pauzeadvies</h3><span class="pill">ruimte: ${Math.max(0, activeBar().maxConcurrentBreaks - getActiveBreaks().length)}</span></div>
...
function renderWarningStrip() {
  const warnings = getActiveBreaks().map((entry) => ({ entry, employee: activeBar().employees.find((item) => item.id === entry.employeeId), diff: minutesBetween(entry.expectedReturnAt, now()) })).filter((item) => item.diff >= -5);
...
function renderBreaks() {
  ...
  const selected = bar.employees.filter((employee) => selectedEmployees.has(employee.id));
  const durations = [15, 30, 45, 60];
  $('#breaks').innerHTML = `
    <div class="card"><div class="card-title"><h2>Pauzes</h2><span class="pill">${active.length}/${bar.maxConcurrentBreaks} actief</span></div><div class="row">${durations.map((duration) => `<button type="button" data-bulk-start="${duration}" ${selected.length ? '' : 'disabled'}>${duration} min</button>`).join('')}</div><p class="muted small">Geselecteerd: ${selected.length ? selected.map((employee) => escapeHtml(employee.name)).join(', ') : 'geen medewerkers'}</p></div>
    ...
    <div class="card"><div class="card-title"><h3>Alle medewerkers</h3><button class="light" type="button" data-clear-selection>Wis selectie</button></div><div class="grid">${bar.employees.length ? bar.employees.map(renderEmployeeBreakRow).join('') : '<div class="empty">Voeg eerst medewerkers toe.</div>'}</div></div>`;
}
...
function renderActiveBreak(entry) {
  const employee = activeBar().employees.find((item) => item.id === entry.employeeId);
...
function renderEmployees(keepFormFocus = false) {
  ...
  const selectedShiftId = bar.shifts.some((shift) => shift.id === draft.shiftId) ? draft.shiftId : bar.shifts[0]?.id || '';
  $('#employees').innerHTML = `<div class="card"><div class="card-title"><h2>Medewerkers</h2><span class="pill">${bar.employees.length}</span></div><form id="employeeForm" class="form"><label>Naam<input name="name" required placeholder="Bijv. Lisa" value="${escapeAttr(draft.name || '')}"></label><label>Shift<select name="shiftId" required>${bar.shifts.map((shift) => `<option value="${shift.id}" ${shift.id === selectedShiftId ? 'selected' : ''}>${escapeHtml(shift.name)} (${shift.startTime}-${shift.endTime})</option>`).join('')}</select></label><button type="submit">Medewerker toevoegen</button></form></div><div class="grid">${bar.employees.length ? bar.employees.map((employee) => { const shift = getShift(employee); return `<div class="employee"><div class="employee-head"><div><div class="employee-name">${escapeHtml(employee.name)}</div><p class="muted small">${escapeHtml(shift?.name || 'Geen shift')} · ${employee.startTime}-${employee.endTime} · ${employee.requiredBreakMinutes} min pauze</p></div><button class="danger" type="button" data-delete-employee="${employee.id}">Verwijder</button></div>${renderBlocks(employee)}${renderHistory(employee.id)}</div>`; }).join('') : '<div class="card empty">Nog geen medewerkers.</div>'}</div>`;
}
...
function renderSettings(keepFormFocus = false) {
  ...
  const event = activeEvent();
  const bar = activeBar();
  const archived = state.events.filter((item) => item.archived);
  $('#settings').innerHTML = `<div class="card"><div class="card-title"><h2>Instellingen</h2><span class="pill">${escapeHtml(event.name)}</span></div><label>Kies evenement<select data-active-event>${activeEvents().map((item) => `<option value="${item.id}" ${item.id === event.id ? 'selected' : ''}>${escapeHtml(item.name)} (${item.date})</option>`).join('')}</select></label><form id="eventForm" class="form"><label>Evenementnaam<input name="name" required value="${escapeAttr(event.name)}"></label><label>Datum<input name="date" type="date" required value="${event.date}"></label><button type="submit">Evenement opslaan</button></form><div class="row"><button class="secondary" type="button" data-archive-event="${event.id}">Archiveer evenement</button></div></div>
    <div class="card"><div class="card-title"><h3>Bar binnen dit evenement</h3><span class="pill">${escapeHtml(bar.name)}</span></div><label>Kies bar<select data-active-bar>${event.bars.map((item) => `<option value="${item.id}" ${item.id === bar.id ? 'selected' : ''}>${escapeHtml(item.name)}</option>`).join('')}</select></label><form id="barNameForm" class="form"><label>Barnaam<input name="name" required value="${escapeAttr(bar.name)}"></label><label>Maximale gelijktijdige pauzes<input name="maxConcurrentBreaks" type="number" min="1" max="40" required value="${bar.maxConcurrentBreaks}"></label><button type="submit">Bar opslaan</button></form></div>
    ...
    <div class="card"><div class="card-title"><h3>Archief evenementen</h3><span class="pill">${archived.length}</span></div>${archived.length ? `<div class="grid">${archived.map((item) => `<div class="history-row"><strong>${escapeHtml(item.name)}</strong><span>${item.date} · ${item.bars.length} bar(s)</span><div class="row"><button type="button" data-restore-event="${item.id}">Terugzetten</button><button class="danger" type="button" data-delete-event="${item.id}">Definitief verwijderen</button></div></div>`).join('')}</div>` : '<div class="empty">Geen gearchiveerde evenementen.</div>'}</div>
    <div class="card"><div class="card-title"><h3>Bars voor ${escapeHtml(event.name)}</h3></div><div class="grid">${event.bars.map((item) => `<button type="button" class="selector-button ${item.id === bar.id ? 'active' : ''}" data-select-bar="${item.id}">${escapeHtml(item.name)}<span>${item.employees.length} medewerkers</span></button>`).join('')}</div><form id="barForm" class="form inline-form"><label>Nieuwe bar<input name="name" required placeholder="Bijv. Wijnbar" value="${escapeAttr(state.ui.drafts.newBarName || '')}"></label><button type="submit">Nieuwe bar</button></form></div>
...
function canStartBreak(employee, duration) {
  const bar = activeBar();
  return remainingMinutes(employee) >= duration && !getActiveBreak(employee.id) && getActiveBreaks().length < bar.maxConcurrentBreaks;
}

function startBreak(employeeId, duration) {
  const bar = activeBar();
  const employee = bar.employees.find((item) => item.id === employeeId);
  if (!employee || !canStartBreak(employee, duration)) return false;
  const startedAt = now();
  bar.breaks.push({ id: uid('break'), employeeId, startedAt: startedAt.toISOString(), plannedMinutes: duration, expectedReturnAt: addMinutes(startedAt, duration).toISOString(), endedAt: null, minutesLate: null, notificationsSent: [], alertsSent: [] });
  state.ui.drafts.lastBreakDuration = duration;
  return true;
}

function returnBreak(breakId) {
  const entry = activeBar().breaks.find((item) => item.id === breakId);
...
function addEmployee(formData) {
  const bar = activeBar();
  const shift = bar.shifts.find((item) => item.id === formData.get('shiftId'));
  const name = formData.get('name').trim();
  if (!shift || !name) return;
  mutate(() => {
    bar.employees.push({ id: uid('employee'), name, shiftId: shift.id, startTime: shift.startTime, endTime: shift.endTime, requiredBreakMinutes: Number(shift.defaultBreakMinutes) });
...
function createNewEvent(formData) {
  ...
  mutate(() => {
    ...
    state.ui.activeBarId = event.bars[0].id;
    state.ui.drafts.newEventName = '';
    state.ui.drafts.employee = { name: '', shiftId: event.bars[0].shifts[0]?.id || '' };
    selectedEmployees.clear();
    showToast('Evenement aangemaakt. Voeg nu bars toe voor dit evenement.');
  });
}
...
function addBar(formData) {
  ...
  mutate(() => {
    ...
    activeEvent().bars.push(bar);
    state.ui.activeBarId = bar.id;
    state.ui.drafts.newBarName = '';
    selectedEmployees.clear();
    showBarSelector = false;
    showToast('Bar aan evenement toegevoegd.');
  });
}

function selectBar(barId) {
  const bar = activeEvent().bars.find((item) => item.id === barId);
  if (!bar) return;
  mutate(() => {
    state.ui.activeBarId = bar.id;
    state.ui.drafts.employee.shiftId = bar.shifts[0]?.id || '';
    selectedEmployees.clear();
    showBarSelector = false;
  });
}
...
function archiveEvent(eventId) {
  ...
  mutate(() => {
    ...
    state.ui.activeEventId = nextEvent.id;
    state.ui.activeBarId = nextEvent.bars[0].id;
    selectedEmployees.clear();
...
function deleteEvent(eventId) {
  const event = state.events.find((item) => item.id === eventId && item.archived);
  if (!event || !confirm(`Evenement "${event.name}" definitief verwijderen? Dit kan niet ongedaan gemaakt worden.`)) return;
  mutate(() => {
    state.events = state.events.filter((item) => item.id !== eventId);
    if (!state.events.length) state.events.push(createEvent('Nieuw evenement'));
    if (!state.events.some((item) => item.id === state.ui.activeEventId)) {
      const nextEvent = state.events.find((item) => !item.archived) || state.events[0];
      state.ui.activeEventId = nextEvent.id;
      state.ui.activeBarId = nextEvent.bars[0].id;
    }
    ...
    try {
      {
        ...
        state = (parsed?.events || parsed?.bars) ? normalizeState(parsed) : migrateV1(parsed);
      }
...
function bindEvents() {
  document.addEventListener('click', (event) => {
    ...
    if (target.dataset.deleteEmployee) mutate(() => { const bar = activeBar(); bar.employees = bar.employees.filter((employee) => employee.id !== target.dataset.deleteEmployee); bar.breaks = bar.breaks.filter((entry) => entry.employeeId !== target.dataset.deleteEmployee); selectedEmployees.delete(target.dataset.deleteEmployee); });
    if (target.dataset.deleteShift) mutate(() => { const bar = activeBar(); if (bar.employees.some((employee) => employee.shiftId === target.dataset.deleteShift)) return showToast('Shift is nog in gebruik.'); bar.shifts = bar.shifts.filter((shift) => shift.id !== target.dataset.deleteShift); });
  ...
  document.addEventListener('change', (event) => {
    ...
    if (event.target.dataset.activeEvent) mutate(() => { state.ui.activeEventId = event.target.value; state.ui.activeBarId = activeEvent().bars[0].id; selectedEmployees.clear(); });
    if (event.target.dataset.activeBar) selectBar(event.target.value);
    const form = event.target.closest('form');
    if (form) updateDraft(form);
  });
  ...
  document.addEventListener('submit', (event) => {
    ...
    if (event.target.id === 'barNameForm') mutate(() => { const data = new FormData(event.target); const bar = activeBar(); bar.name = data.get('name').trim(); bar.maxConcurrentBreaks = Number(data.get('maxConcurrentBreaks')); showToast('Bar opgeslagen.'); });
  });
